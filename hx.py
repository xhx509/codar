...
def getsst(ask_input):
    ...
    #else:
    url='http://tds.maracoos.org/thredds/dodsC/SST-'+str(year)+'-Agg.nc?lat[0:1:3660],lon[0:1:4499],'+'mcsst['+str(index_second)+':1:'+str(index_second)+'][0:1:3660][0:1:4499]'+',time['+str(index_second)+':1:'+str(index_second)+']'
    try:
        print url
        dataset=open_url(url)
    except:
...
def plot_getsst(ask_input,utc,gbox):
  ...
  CS = plt.contourf(X,Y,sst_part[0],np.arange(mi-0.05,ma+0.05,(ma-mi+0.1)/12.))
...
'''
def getobs_driftid_byrange(gbox,input_time):
